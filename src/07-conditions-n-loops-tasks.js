...
function isBracketsBalanced(str) {
  ...
  [...str].forEach((char) => {
    if (openArr.includes(char)) {
      stack.push(char);
    } else if (closeArr.includes(char)) {
      const open = openArr[closeArr.indexOf(char)];
      const close = char;

      if (stack.slice(-1)[0] === open) {
        stack.pop();
      } else {
        stack.push(close);
      }
...
function getMatrixProduct(m1, m2) {
  return m1
    .map((row, i) => m2[0].map((_, j) => row.reduce((sum, __, k) => sum + m1[i][k] * m2[k][j], 0)));
}
...
function evaluateTicTacToePosition(position) {
  ...
  const result = win.find((item) => {
    const [a, b, c, d, e, f] = item;
    return position[a][b] && position[a][b] === position[c][d] && position[c][d] === position[e][f];
  });
  return result ? position[result[0]][result[1]] : undefined;
}
